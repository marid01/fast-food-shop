...
import {fetchBurger, fetchCategories} from "../../redux/reducers/products";
...
function Home() {
    const itemsProducts = useSelector(({products}) => products.items);
    const itemsCategory = useSelector(({products}) => products.itemsCategory);
    ...
    const getBurgers = useCallback(() => {
        ...
        const arrBurger = itemsProducts.filter(({id}) => cat?.includes(id))

        setBurgers(arrBurger)
    }, [itemsProducts, itemsCategory])
    ...
    const getTwister = useCallback(() => {
        ...
        const arrTwister = itemsProducts.filter(({id}) => cat?.includes(id))
    ...
    const getChicken = useCallback(() => {
        ...
        const arrChicken = itemsProducts.filter(({id}) => cat?.includes(id))
    ...
    return (
        <div className="main">
           ...
           <div className="menu">
            ...
            <div className="content__items-burger" id="burgers">
                <div className="container">
                    <div className="content__items-burger__inner">
                        {isLoaded ? burgers?.map((obj) => {
                            return <BurgerBlock
            ...
            <div className="content__items-twister" id="twister">
                <div className="container">
                    <div className="content__items-twister__inner">
                        {isLoaded ? twister?.map((obj) => {
                            return <TwisterBlock
                                onClickAddItems={handleAddItemsToBasket}
                                addedCount={basketItems[obj.id] && basketItems[obj.id].items.length}
                                {...obj}/>
            ...
            <div className="content__items-chicken" id="chicken">
                <div className="container">
                    <div className="content__items-chicken__inner">
                        {isLoaded ? chicken?.map((obj) => {
                            return <ChickenBlock
                                onClickAddItems={handleAddItemsToBasket}
                                addedCount={basketItems[obj.id] && basketItems[obj.id].items.length}
                                {...obj}/>
